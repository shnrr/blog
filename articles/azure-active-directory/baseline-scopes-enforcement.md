...
継続的に無効にすることは推奨していないため、一時的な問題の回避として、6 章で案内した確認・対処を進めるための時間を確保する目的で、変更の影響を受ける以前の動作に戻すことが有効です。

### 以前の動作に戻す方法
1. 以下の URL にアクセスし、条件付きアクセスのベースライン スコープ設定画面を開きます。現在はこの URL からのみアクセスが可能です。
   https://aka.ms/BaselineScopesSettingsUX
2. 条件付きアクセス管理者以上の権限を持つアカウントでサインインします。
3. 画面上に表示される以下の選択肢を選択します。
   - [適用を無効にする (Disable enforcement)]
4. 保存して設定を反映します。
5. 通常は数分間で動作が以前の動作に戻る想定です。改めてアプリケーションへのサインインを試します。
