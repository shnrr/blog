...
> [!NOTE]
> 2026 年 5 月 7 日更新: カスタム アプリケーション作成時の [サポートされているアカウントの種類] を更新しました
> 2026 年 5 月 26 日更新: カスタム アプリケーション作成時の [サポートされているアカウントの種類] を改めて更新しました
> 2026 年 8 月 4 日更新: 変更の影響を受けた場合の一時的な回避策を追記しました。
...
> [!NOTE]
> 以前、この手順 3 においてサポートされているアカウントの種類は「複数の Entra ID テナント」を選択する必要がありました。
> これは一部アプリケーションで限定的に発生することが確認されていた問題に対する回避策でした。
> 現在は問題が解消していますので、新しく構成する場合には「この組織ディレクトリのみに含まれるアカウント (シングル テナント)」を選択してください。
> もし、既に「複数の Entra ID テナント」を選択して構成していた場合、以下の手順で「この組織ディレクトリのみに含まれるアカウント」に変更することをご検討ください。
> 
> 1. Azure ポータルもしくは Microsoft Entra 管理センターにアプリケーションを更新可能な管理者アカウント (所有者、アプリケーション管理者など) でサインインします
> 2. [Microsoft Entra ID] > [アプリの登録] に遷移します。
> 3. 作成済みの [アプリの登録] を開きます。
> 4. Authentication (Preview) を開きます。
> 5. Supported accounts を選択して、「サポートされているアカウントの種類:」を「シングル テナントのみ - テナント名」を選択します
>
> この変更によりベースライン スコープや条件付きアクセス ポリシーの動作に変更はありません。
> マルチ テナントであっても動作上の違いはないのですが、作成するアプリの目的はシングル テナントにのみ影響を与える用途であるため、「シングル テナントのみ」であることが妥当であると考えられます。
...
## 7. もし準備をする前に影響を受けてしまったら
変更の展開前に準備が間に合わず、すでに影響を受けてしまった場合には、今まで適用されていなかった条件付きアクセス ポリシーによってアクセスがブロックされてしまう場合があります。
その場合、まずは変更の影響を受ける以前の動作に戻し、その後に対処を進めることをご検討ください。
継続的に無効にすることは推奨していないため、一時的な問題の回避として、6 章でご案内した確認・対処を進めるための時間を確保する目的で、変更の影響を受ける以前の動作に戻すことが有効です。

### 以前の動作に戻す方法
1. 以下の URL にアクセスし、条件付きアクセスのベースライン スコープ設定画面を開きます。現在はこの URL からのみアクセスが可能です。
   https://aka.ms/BaselineScopesSettingsUX
2. 条件付きアクセス管理者以上の権限を持つアカウントでサインインします。
3. 画面上に表示されるベースライン スコープ設定画面で以下の選択肢を選択します。
   - [適用を無効にする (Disable enforcement)]
4. 保存して設定を反映します。
5. 通常は数分間以内に以前の動作に戻る想定です。改めてアプリケーションへのサインインを試します。

> [!NOTE]
> このオプションは、継続的に無効にすることを推奨するものではありません。あくまで一時的な問題の回避として、1~6 章でご案内した確認・対処を進めるための時間を確保するためにご利用ください。
